...
func (mc *MigrationController) syncGameServer(ctx context.Context, key string) error {
	...
	if pod.Spec.NodeName != gs.Status.NodeName || !mc.addressMatches(node, gs) {
		gsCopy := gs.DeepCopy()
...
func (mc *MigrationController) addressMatches(node *k8sv1.Node, gs *agonesv1.GameServer) bool {
	...
	mc.baseLogger.Warnf("address mismatch for gs=%s: gs.Status.Address=%s, node addresses=%s",
